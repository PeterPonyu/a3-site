...
test.describe('a3-site Pages landing', () => {
  test('Home responds HTTP 200', async ({ page }) => {
    const response = await page.goto(A3_PAGES_URL, { waitUntil: 'domcontentloaded' });
    expect(response?.status(), `${A3_PAGES_URL} should return 200`).toBe(200);
  });

  test('Home is a code leaf: no unpublished-result tokens or figures', async ({ page }) => {
    await page.goto(A3_PAGES_URL, { waitUntil: 'domcontentloaded' });
    const html = await page.content();
    const text = await visibleBodyText(page);
    expect(findResultLeaks(`${text}\n${html}`), 'unpublished result tokens on landing').toEqual([]);
    expect(html, 'landing must not embed figures').not.toMatch(/<img\b/i);
  });

  test('Sticky Homepage + SCPortal chrome', async ({ page }) => {
    await page.goto(A3_PAGES_URL, { waitUntil: 'networkidle' });
  ...
  test('No journal-submission packaging', async ({ page }) => {
    ...
    const html = await page.content();
    const text = await visibleBodyText(page);
    expect(findSubmissionPackagingLeaks(text), `submission packaging leaks on ${A3_PAGES_URL}`).toEqual([]);
    expect(hasBibTeXKit(html), 'BibTeX kit present on unpublished leaf').toBe(false);
  });

  test('Public code href is the a3-site GitHub repo', async ({ page }) => {
    await page.goto(A3_PAGES_URL, { waitUntil: 'networkidle' });
    const codeLink = page.locator(`a[href="${A3_GITHUB_URL}"]`).first();
    await expect(codeLink, 'Code link to PeterPonyu/a3-site missing').toBeVisible();
  });
  ...
  for (const url of LANDING_ROUTES) {
    test(`Bookmark landing stays figure-free 200: ${new URL(url).pathname}`, async ({ page }) => {
      const response = await page.goto(url, { waitUntil: 'domcontentloaded' });
      expect(response?.status(), `${url} must stay 200 landing, not a restored results page`).toBe(200);
      const html = await page.content();
      const text = await visibleBodyText(page);
      const combined = `${text}\n${html}`;
      expect(findResultLeaks(combined), `unpublished result tokens on ${url}`).toEqual([]);
      expect(html, `${url} must not embed figures`).not.toMatch(/<img\b/i);
      const codeLink = page.locator(`a[href="${A3_GITHUB_URL}"]`).first();
      await expect(codeLink, `Code link missing on ${url}`).toBeVisible();
    });
  }
});
